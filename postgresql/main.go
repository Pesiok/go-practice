...
// docker run --name go-postgres --rm -p 127.0.0.1:5432:5432 -e POSTGRES_PASSWORD=password -e POSTGRES_USER=kek -e POSTGRES_DB=bookstore -d postgres
...
func init() {
	var err error
	db, err = sql.Open("postgres", "postgres://kek:password@localhost/bookstore?sslmode=disable")
	if err != nil {
		panic(err)
	}
